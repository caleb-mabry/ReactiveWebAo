...
import { addCharacter, addMessage, addOocMessage, setAreaNames, setAreaPlayerCount, setAssetUrl, setBackground, setCharacterAvailablility, setClient, setTimers } from "../features/masterserver/clientserver";
...
const Client = () => {
        ...
        switch (informationCode) {
            case 0:
                ...
                areas.forEach((playerCount, indx) => {
                    dispatch(setAreaPlayerCount({areaNumber: indx, playerCount: playerCount}))
                })
                break;
    ...
    const scHandler = (msg) => {
        ...
        const characters = characterNames.map(char => {return({name: char})})
