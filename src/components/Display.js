...
const getBackground = (assetUrl, backgroundName) => {
    ...
    return assetUrl + "background/" + encodeURI(backgroundName.toLowerCase()) + "/defenseempty.png"
}
const sideUrl = (assetUrl, message, backgroundName) =>{
    if (backgroundName === undefined || message === undefined) {
        return ''
    }
    return assetUrl + "background/" +  encodeURI(backgroundName.toLowerCase()) + '/' + positions[message.side].desk.ao2
}
...
const getBackgroundWithPosition = (assetUrl, backgroundName, positionBackgroundName) => {
    return assetUrl + "background/" + encodeURI(backgroundName.toLowerCase()) + "/" + positionBackgroundName
}
const DisplayBackground = ({messages, assetUrl, backgroundName}) => {
    ...
    if (Object.keys(positions).includes(message.side)) {
        return (<>
                <InsideImage src={getBackgroundWithPosition(assetUrl, backgroundName, positions[message.side].bg)} />
...
const Display = () => {
    ...
    return (
        <div>
            ...
            <CharacterDisplay>
                <DisplayBackground messages={messages} assetUrl={assetUrl} backgroundName={backgroundName}/>
